...
class EntuEksponaat(BaseModel):
    ...
    _parent: Optional[str] = Field(None, description="Parent entity reference")
...
class MuisEvent(BaseModel):
    ...
    dateering_algus: Optional[str] = Field(None, description="Start date (aaaa or kk.aaaa or pp.kk.aaaa)")
    ...
    osaleja: Optional[str] = Field(None, description="Participant (Lastname, Firstname OR org name)")
    osaleja_roll: Optional[str] = Field(None, description="Participant role, required if osaleja filled")
    kihelkond: Optional[str] = Field(None, description="Parish")
    
    @root_validator
    def validate_event_dependencies(cls, values):
        """Validate conditional field requirements"""
        # If osaleja filled, osaleja_roll is required
        if values.get('osaleja') and not values.get('osaleja_roll'):
            raise ValueError("osaleja_roll required when osaleja is provided")
        
        # If kohanimi filled, selgitus required
        if values.get('kohanimi') and not values.get('selgitus'):
            raise ValueError("selgitus required when kohanimi is provided")
        
        # If dateering_lopp filled, dateering_algus must be filled
        if values.get('dateering_lopp') and not values.get('dateering_algus'):
        ...
        if (values.get('eesti_admin_yksus') or values.get('kihelkond')) and values.get('riik') != 'Eesti':
            values['riik'] = 'Eesti'
...
class MuisMuseaal(BaseModel):
    ...
    # ========================================================================
    acr: str = Field(..., description="Archive code, e.g., 'VBM'", regex=r'^[A-Z]{2,4}$')
    trt: str = Field('_', description="Separator, always '_'", regex=r'^_$')
    ...
    kogusse_registreerimise_aeg: Optional[str] = Field(
        None, 
        description="Registration date, format: pp.kk.aaaa",
        regex=r'^\d{2}\.\d{2}\.\d{4}$'
    )
    yleandja: Optional[str] = Field(
        None, 
        description="Donor/transferor (Lastname, Firstname format or MuIS admin ID)"
    )
    ...
    @root_validator
    def validate_measurement_dependencies(cls, values):
        """Validate measurement field dependencies"""
        for i in range(1, 5):
            parameeter = values.get(f'parameeter_{i}')
            yhik = values.get(f'yhik_{i}')
            vaartus = values.get(f'vaartus_{i}')
    ...
    @root_validator
    def validate_material_dependencies(cls, values):
        """Validate material comment dependencies"""
        for i in range(1, 4):
            materjal = values.get(f'materjal_{i}')
            kommentaar = values.get(f'materjali_{i}_kommentaar')
    ...
    @root_validator
    def validate_technique_dependencies(cls, values):
        """Validate technique comment dependencies"""
        for i in range(1, 4):
            tehnika = values.get(f'tehnika_{i}')
            kommentaar = values.get(f'tehnika_{i}_kommentaar')
    ...
    @root_validator
    def validate_event_1_dependencies(cls, values):
        """Validate Event 1 field dependencies"""
        osaleja = values.get('osaleja_1')
        osaleja_roll = values.get('osaleja_roll_1')
        kohanimi = values.get('toimumiskoha_tapsustus_kohanimi_1')
        selgitus = values.get('toimumiskoha_tapsustus_selgitus_1')
        dateering_algus = values.get('dateering_algus_1')
        dateering_lopp = values.get('dateeringu_lopp_1')
        admin_yksus = values.get('eesti_admin_yksus_1')
        kihelkond = values.get('kihelkond_1')
        riik = values.get('riik_1')
        
        # If osaleja filled, role required
        if osaleja and not osaleja_roll:
            raise ValueError("osaleja_roll_1 required when osaleja_1 is filled")
        
        # If kohanimi filled, selgitus required
        if kohanimi and not selgitus:
            raise ValueError("selgitus_1 required when kohanimi_1 is filled")
        
        # If dateering_lopp filled, dateering_algus required
        if dateering_lopp and not dateering_algus:
            raise ValueError("dateering_algus_1 required when dateering_lopp_1 is filled")
        
        # If admin_yksus or kihelkond filled, riik must be 'Eesti'
        if (admin_yksus or kihelkond) and riik != 'Eesti':
            values['riik_1'] = 'Eesti'
        
        return values
    
    @root_validator
    def validate_event_2_dependencies(cls, values):
        """Validate Event 2 field dependencies (same as Event 1)"""
        osaleja = values.get('osaleja_2')
        osaleja_roll = values.get('osaleja_roll_2')
        kohanimi = values.get('toimumiskoha_tapsustus_kohanimi_2')
        selgitus = values.get('toimumiskoha_tapsustus_selgitus_2')
        dateering_algus = values.get('dateering_algus_2')
        dateering_lopp = values.get('dateeringu_lopp_2')
        admin_yksus = values.get('eesti_admin_yksus_2')
        kihelkond = values.get('kihelkond_2')
        riik = values.get('riik_2')
    ...
    @root_validator
    def validate_description_dependencies(cls, values):
        """Validate description field dependencies"""
        # Description 1
        tyyp_1 = values.get('teksti_tyyp_1')
        tekst_1 = values.get('tekst_1')
        if tyyp_1 and not tekst_1:
            raise ValueError("tekst_1 required when teksti_tyyp_1 is filled")
        if tekst_1 and not tyyp_1:
            raise ValueError("teksti_tyyp_1 required when tekst_1 is filled")
        
        # Description 2
        tyyp_2 = values.get('teksti_tyyp_2')
        tekst_2 = values.get('tekst_2')
        if tyyp_2 and not tekst_2:
            raise ValueError("tekst_2 required when teksti_tyyp_2 is filled")
        if tekst_2 and not tyyp_2:
            raise ValueError("teksti_tyyp_2 required when tekst_2 is filled")
        
        return values
    
    @root_validator
    def validate_alternative_name_dependencies(cls, values):
        """Validate alternative name dependencies"""
        tyyp = values.get('nimetuse_tyyp')
        nimetus = values.get('alt_nimetus')
        
        if tyyp and not nimetus:
            raise ValueError("alt_nimetus required when nimetuse_tyyp is filled")
        if nimetus and not tyyp:
            raise ValueError("nimetuse_tyyp required when alt_nimetus is filled")
        
        return values
    
    @root_validator
    def validate_alternative_number_dependencies(cls, values):
        """Validate alternative number dependencies"""
        tyyp = values.get('numbri_tyyp')
        number = values.get('alt_number')
        
        if tyyp and not number:
            raise ValueError("alt_number required when numbri_tyyp is filled")
        if number and not tyyp:
            raise ValueError("numbri_tyyp required when alt_number is filled")
        
        return values
    
    @root_validator
    def validate_condition_damage_dependency(cls, values):
        """If condition is 'halb' or 'väga halb', kahjustused is required"""
        seisund = values.get('seisund')
        kahjustused = values.get('kahjustused')
        
        if seisund in ['halb', 'väga halb'] and not kahjustused:
    ...
    def validate_reference_dependency(cls, values):
        ...
        tyyp = values.get('viite_tyyp')
        vaartus = values.get('viite_vaartus')
        
        if vaartus and not tyyp:
            raise ValueError("viite_tyyp required when viite_vaartus is filled")
        
        return values
    
    @root_validator
    def validate_archaeology_dependency(cls, values):
        """If leiu_liik filled, leiukontekst is required"""
        leiukontekst = values.get('leiukontekst')
        leiu_liik = values.get('leiu_liik')
        
        if leiu_liik and not leiukontekst:
...
def create_muis_header() -> List[List[str]]:
    ...
    row_1 = [
        '', '', '', 
        'Number', '', '', '', '', '', '', '', '',
        'Nimetus', 'Püsiasukoht', 'Tulmelegend', 'Originaal ?',
        'Vastuvõtt', '', '', '', '',
        'Mõõdud', '', '', '', '', '', '', '', '', '', '', '',
        'Materjal 1', 'Materjali 1 kommentaar',
        'Materjal 2', 'Materjali 2 kommentaar',
        'Materjal 3', 'Materjali 3 kommentaar',
        'Värvus',
        'Tehnika 1', 'Tehnika 1 kommentaar',
        'Tehnika 2', 'Tehnika 2 kommentaar',
        'Tehnika 3', 'Tehnika 3 kommentaar',
        'Olemus 1', 'Olemus 2',
        'Viited', '',
        ...
        'Museaal sündmuses 1', '', '', '', '', '', '', '', '', '', '',
        'Museaal sündmuses 2', '', '', '', '', '', '', '', '', '', '',
        'Avalik?', 'Avalikusta praegused andmed?',
        'Kirjeldus', '', '', '',
        'Teised nimetused', '',
        'Teised numbrid', ''
    ]
    ...
    row_2 = [
        'museaali_ID', 'Importimise staatus', 'Kommentaar',
        'Acr', 'Trt', 'Trs', 'Trj', 'Trl', 'Kt', 'Ks', 'Kj', 'Kl',
        '', '', '', '',
        'Vastuvõtu nr', 'Esmane üldinfo', 'Kogusse registreerimise aeg', 'Üleandja', 'Muuseumile omandamise viis',
        'Parameeter 1', 'Ühik 1', 'Väärtus 1',
        'Parameeter 2', 'Ühik 2', 'Väärtus 2',
        'Parameeter 3', 'Ühik 3', 'Väärtus 3',
        'Parameeter 4', 'Ühik 4', 'Väärtus 4',
        '', '', '', '', '', '', '',
        '', '', '', '', '', '',
        '', '',
        'Viite tüüp', 'Väärtus',
        'Leiukontekst', 'Leiu liik',
        'Pealkirja keel', 'Ainese keel',
        '', '',
        'Sündmuse liik', 'Toimumiskoha täpsustus: kohanimi', 'Toimumiskoha täpsustus: selgitus',
        'Dateering/ dateeringu algus', 'On ekr', 'Dateeringu lõpp',
        'Riik', 'Eesti admin üksus', 'Osaleja', 'Osaleja roll', 'Kihelkond',
        'Sündmuse liik', 'Toimumiskoha täpsustus: kohanimi', 'Toimumiskoha täpsustus: selgitus',
        'Dateering/ dateeringu algus', 'On ekr', 'Dateeringu lõpp',
        'Riik', 'Eesti admin üksus', 'Osaleja', 'Osaleja roll', 'Kihelkond',
        '', '',
        'Teksti tüüp 1', 'Tekst 1', 'Teksti tüüp 2', 'Tekst 2',
        'Nimetuse tüüp', 'Nimetus',
        'Numbri tüüp', 'Number'
    ]
    
    # Row 3: Validation rules (simplified - full text in reference file)
    row_3 = [
        'Täidab süsteem', 'Täidab süsteem', 'Täidab süsteem',
        'Peab vastama MuISis olevale ACR-ile',
        'Peab vastama MuISis olevale TRT-le',
        'Peab olema number', 'Peab olema number', '',
        'Peab vastama MuISis olevale KT-le',
        'Peab olema number', 'Peab olema number', '',
        'Peab olema MuISi asukohapuus olemas; püsiasukoha järgi täidetakse automaatselt jooksev asukoht',
        ...
        'Täidetakse kujul aaaa või kk.aaaa või pp.kk.aaaa; Käsitletakse täpse dateeringuna, kui dateeringu lõpp ei ole täidetud',
        'y (on eKr) või tühi (on pKr)',
        'Täidetakse, kui dateeringut soovitakse esitada ajavahemikuna; Täidetakse kujul aaaa või kk.aaaa või pp.kk.aaaa',
        'Peab olema "Eesti", kui "Eesti admin üksus" või "Kihelkond" on täidetud', '',
        'Peab olema MuISis ajalooline osaleja; täidetakse kujul Perekonnanimi, Eesnimi VÕI Organisatsiooni nimi (või osaleja ID)',
        ...
        'Peab olema MuISis ajalooline osaleja; täidetakse kujul Perekonnanimi, Eesnimi VÕI Organisatsiooni nimi (või osaleja ID)',
        'Kohustuslik, kui osaleja on täidetud', '',
        'y (on avalik) või tühi', 'y (avalikusta praegused andmed) või tühi',
